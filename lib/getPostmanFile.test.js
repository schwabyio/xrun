...
test('properly retrieve postmanFile for a valid filePath', async() => {
    const filePath = path.join(__dirname, 'unit-tests', 'test-data', 'test-suites-project1', 'directory2', 'XTEST_DEMO_EXPECT_RESPONSE_BODY_TO_NOT_HAVE_RESPONSE_PROPERTY.postman_collection.json')
...
test('properly throws error for a non json file format', async() => {
    const filePath = path.join(__dirname, 'unit-tests', 'test-data', 'test-suites-project1', 'directory2', 'text-type-file.txt')
