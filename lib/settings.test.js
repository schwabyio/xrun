...
test('init(): properly returns settings', async() => {
    ...
    const settings = await setting.init(settingsPath, process.cwd())
...
test('init(): properly returns error for invalid property: nat-not-zero', async() => {
    ...
    await expect(setting.init(settingsPath, process.cwd())).rejects.toThrow(/validation error: timeoutCollection: value must be a positive integer and also not zero: /)
})
...
test('init(): properly returns error for invalid property: boolean', async() => {
    ...
    await expect(setting.init(settingsPath, process.cwd())).rejects.toThrow(/validation error: autoOpenTestResultHtml: value is not a Boolean type/)
})
...
test('init(): properly returns error for invalid property: string', async() => {
    ...
    await expect(setting.init(settingsPath, process.cwd())).rejects.toThrow(/validation error: slackChannel: value is not a String type/)
})


test('init(): properly returns settings with injected xRunProjectPath matching process.cwd()', async() => {
    const settingsPath = path.join(__dirname, 'unit-tests/test-data/test-settings/test-settings-invalid-non-empty-string.json')
    const settings = await setting.init(settingsPath, process.cwd())

    expect(settings).toHaveProperty("xRunProjectPath", process.cwd())
})
...
test('init(): properly returns error for invalid property: non-empty-no-spaces-string', async() => {
    ...
    await expect(setting.init(settingsPath, process.cwd())).rejects.toThrow(/validation error: projectName: value must not contain any whitespace characters:/)
})
...
test('init(): properly returns error for invalid settingsPath', async() => {
    ...
    await expect(setting.init(settingsPath, process.cwd())).rejects.toThrow(/Oops, unable to load settings file /)
})
