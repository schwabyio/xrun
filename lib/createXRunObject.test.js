...
test('properly returns postmanFiles', async() => {
    ...
    //Validate top level
    expect(Object.keys(xRunObject)).toHaveLength(2)
})
...
test('returns proper error when directoryList does not contain any valid postman collections', async() => {
    ...
    const path1 = path.join(__dirname, '/unit-tests/test-data/test-project1', 'empty-directory')
    const path2 = path.join(__dirname, '/unit-tests/test-data/test-project1', 'invalid-postman-file')
