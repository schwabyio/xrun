...
test('properly writes a file', async() => {
    const filePath = path.join(__dirname, 'unit-tests', 'test-data', 'test-project2', 'throwaway', 'test-file.txt')
...
test('properly throws error for bad filePath', async() => {
    const badFilePath = path.join(__dirname, 'unit-tests', 'test-data', 'test-project1', 'this-directory-does-not-exist', 'file-should-not-be-created.txt')
    const data = 'Creating this test file.'
