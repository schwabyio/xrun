...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//    xrunner-lib.js - Library for xRunner.                                   //
//                                                                            //
...
var xRunnerLib = function xRun() {
  ...
  self.getDirectoryList = function getDL(callback) {
    ...
    const suitesParentDirectory = 'xrunner'

    //Set suitesPath
    const suitesDirectoryPath = path.join(self.settings.xRunnerProjectPath, suitesParentDirectory)
    ...
    fs.access(suitesDirectoryPath, function (err) {
      if (err) {
        errMsg = `Oops, there must be a directory named '${suitesParentDirectory}' within ${self.settings.xRunnerProjectPath} that contains a valid 'suites.json' file.`
            ...
            if (tempDirectoryList.length === 0) {
              ...
              tempDirectoryList.forEach(function(directory) {
                if (path.isAbsolute(directory) === true) {
                  ...
                  directoryList.push(path.join(self.settings.xRunnerProjectPath, directory))
                }
              })
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  self.run = function runTests(xRunnerObject, commandLine, callback) {
    ...
    testResultSummary['xRunnerVersion'] = version
    ...
    testResultSummary['totalNumberOfCollections'] = xRunnerObject['postmanCollections'].length
    ...
    printCommandLineHeader(xRunnerObject, commandLine)
        ...
        if (err) {
          ...
          fs.mkdir(testResultsHtmlPath, { recursive: true }, function resultOfMkdirp(err) {
            if (err) {
              ...
              eachLimit(xRunnerObject.postmanCollections, limitConcurrency, function iterateOverObject(item, callback) {
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  self.createXRunnerObject = function createXRunnerObject(directoryList, callback) {
    const xRunnerObject = {}
    const environmentType = self.settings.environmentType
    var xRunnerCollectionObject = {}
    var errMsg = ''


    //Initialize xRunnerObject['postmanCollections']
    xRunnerObject['postmanCollections'] = []
      ...
      if (errMsg) {
        ...
        loadAllPostmanFilesToObject(directoryList, function resultOfLoadAllPostmanFiles(errMsg, postmanFilesObject) {
          if (errMsg) {
            ...
            //Set xRunnerCollectionObject for directory
            Object.keys(postmanFilesObject).forEach(function resultOfPostmanObject(directory) {
              Object.keys(postmanFilesObject[directory]).forEach(function resultOfPostmanObject(file) {
                if (file.getPostmanFileType() === 'postman_collection') {
                  xRunnerCollectionObject = {}

                  xRunnerCollectionObject['globalVariables'] = postmanGlobalVariableObject
                  xRunnerCollectionObject['environmentVariables'] = postmanEnvironmentVariableObject
                  xRunnerCollectionObject['directoryName'] = path.basename(directory)
                  xRunnerCollectionObject['collection'] = postmanFilesObject[directory][file]

                  //Add xRunnerCollectionObject to xRunnerObject
                  xRunnerObject['postmanCollections'].push(xRunnerCollectionObject)
                } else {
            ...
            return callback(null, xRunnerObject)
          }
        })
      }
    })
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  self.getCollectionInfo = function getCI(xRunnerObject, callback) {
    ...
    xRunnerObject.postmanCollections.forEach(function (postmanCollection) {
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  self.filterXRunnerObjectFromCollectionList = function resultOfFilter(xRunnerObject, collectionCSVList, callback) {
    var errMsg = ''
    ...
    //First confirm that each collection in collectionList exists in xRunnerObject
    ...
    //Remove all collections in xRunnerObject not provided within collectionList (iterate in reverse to delete array items)
    collectionIndex = xRunnerObject.postmanCollections.length
    while (collectionIndex--) {
      //Reset collectionFoundFlag
      collectionFoundFlag = false

      collectionList.forEach(function (collectionName) {
        if (xRunnerObject.postmanCollections[collectionIndex].collection.info.name === collectionName) {
          collectionFoundFlag = true
        }
      })

      //If collection not found delete from xRunnerObject
      if (collectionFoundFlag === false) {
        xRunnerObject.postmanCollections.splice(collectionIndex, 1)
      }
    }
  ...
  function populateEnvironmentVariables(postmanEnvironmentVariableObject, directoryList, environmentType, callback) {
    ...
    directoryList.forEach(function resultOfArrayDirectory(directory) {
      //Check environment bulk files from xrunner directory
      potentialEnvironmentVariableBulkFilePath = path.join(directory, '../xrunner/', environmentType + '.bulk')
  ...
  function createCollectionHtmlReport(testResultsHtmlPath, collectionResults, callback) {
    ...
    const collectionTemplatePath = path.join(__dirname, 'html-templates', 'template-xrunner-collection.hbs')
  ...
  function constructPostmanOptions(postmanCollection) {
    ...
    //options['reporter']['html'] = { export : path.join(testResultsBasePath, 'html', collectionName + '.html'), template: path.join(__dirname, 'html-templates', 'template-xrunner-collection.hbs') }
  ...
  function constructCollectionResults(postmanCollection, summary) {
    ...
    collectionResults.xRunnerVersion = version
  ...
  function initializePostmanVariableObject(variableType) {
    ...
    if (variableType === 'environment') {
      name = 'xRunner Environment Variables'
      postmanVariableScope = 'environment'
    } else if (variableType === 'globals') {
      name = 'xRunner Global Variables'
      postmanVariableScope = 'environment'
    }
  ...
  function createSummaryHtml(testResultSummary, testResultsHtmlPath, callback) {
    ...
    const summaryTemplatePath = path.join(__dirname, 'html-templates', 'template-xrunner-summary.hbs')
  ...
  function uploadTestResultsToWebserver(testResultsHtmlPath, callback) {
    ...
    const zipFileName = 'xrunner-test-results.zip'
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  function printCommandLineHeader(xRunnerObject, commandLine) {
    var line = ''
    ...
    if (commandLine === true) {
      line = EOL +
             ...
             '                                                                       xRunner Version ' + version + EOL +
             '__________________________________________________________________________________________________________________________________________________________________________' + EOL + EOL +
             'Settings: ' + EOL +
             'Project Name: ' + self.settings.projectName + EOL +
             'Suite ID: ' + self.settings.suiteId + EOL +
             'Total Number of Collections to Run: ' + xRunnerObject.postmanCollections.length + EOL +
