...
const runTests = function runTests() {
    ...
    self.determineTestResultsBasePath = function determineTestResultsBasePath(settings, userDefinedSettingsPath) {

      if (settings.testResultsBasePath !== '') {
        return settings.testResultsBasePath
      } else {
        if (userDefinedSettingsPath !== null) {
          const basePath = path.dirname(userDefinedSettingsPath)
          return path.join(basePath, 'test-results')
        } else {
          return null
        }
      }
    ...
    self.run = async function run(settings, xRunObject, userDefinedSettingsPath) {
      ...
      const testResultsBasePath = self.determineTestResultsBasePath(settings, userDefinedSettingsPath)
      const testResultsProjectPath = path.join(testResultsBasePath, settings.projectName)
      const testResultsJUnitBasePath = path.join(testResultsProjectPath, 'junit')
